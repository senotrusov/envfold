...
echo "BASH: Running Error Handling Tests (Strict: ${ENVFLD_TEST_STRICT:-1})"
...
assert_error() {
  ...
  local output code
  if output=$(bin/envfold -c "$conf_file" hook bash 2>&1 >/dev/null); then
    code=0
  else
    code=$?
  fi
...
assert_error_output_empty() {
  ...
  local stdout_output
  stdout_output=$(bin/envfold -c "$conf_file" hook bash 2>/dev/null) || true
...
source <(bin/envfold -c test/test.conf hook bash)

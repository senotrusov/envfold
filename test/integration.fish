...
function assert_error
  ...
  set -l output (bin/envscope -c "$conf_file" hook fish 2>&1 >/dev/null)
  set -l code $status
...
function assert_error_output_empty
  ...
  set -l stdout_output (bin/envscope -c "$conf_file" hook fish 2>/dev/null)
...
bin/envscope -c test/test.conf hook fish | source
